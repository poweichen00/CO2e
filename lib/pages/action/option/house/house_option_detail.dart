...
import '../option.dart';
import '../house/house_option_list.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import '/auth/firebase_auth/auth_util.dart';

class HouseOptionDetail extends StatefulWidget {
  final int optionId;
  final HouseOptionList houseOptionList;

  HouseOptionDetail({
    required this.optionId,
    required this.houseOptionList,
  });

  @override
  _HouseOptionDetailState createState() => _HouseOptionDetailState();
}

class _HouseOptionDetailState extends State<HouseOptionDetail> {
  Future<void> _saveToFirebase(Option option) async {
    try {
      ...
      debugPrint('Error adding option to Firebase: $e');
      showDialog(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('錯誤'),
  ...
  Widget build(BuildContext context) {
    final Option? option =
        widget.houseOptionList.getOptionById(widget.optionId);
      ...
      body: Container(
        ...
        child: Column(
          ...
          children: [
            Container(
              decoration: BoxDecoration(
                boxShadow: [
                  BoxShadow(
                    color:
                        const Color.fromARGB(255, 13, 96, 31).withOpacity(0.5),
                    spreadRadius: 2,
                    blurRadius: 5,
            ...
            Container(
              height: 2,
              color: const Color.fromARGB(115, 162, 161, 161),
